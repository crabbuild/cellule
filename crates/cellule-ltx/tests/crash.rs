use cellule_ltx::{Db, Limits, LocalSegment, SegmentInfo, VerifiedPlan, restore_exact};
...
#[test]
fn crash_writer() {
    let Some(path) = std::env::var_os("CELLULE_LTX_CRASH_TEST_DIR") else {
        return;
    };
...
fn captured_sql_survives_process_kill_and_source_directory_loss() {
    ...
    let child = Command::new(std::env::current_exe().unwrap())
        ...
        .env("CELLULE_LTX_CRASH_TEST_DIR", source.path())
        .stdin(Stdio::piped())
        .stdout(Stdio::piped())
