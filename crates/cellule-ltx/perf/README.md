# `cellule-ltx` versus `celld-ltx`

The results below were measured on the original Crab LTX implementation before
its extraction into Cellule. The harness now builds the extracted crate; run it
again before making performance claims about a new Cellule revision.
...
Cargo workspace: `cellule-ltx` uses `rusqlite` 0.34 while the pinned Celld source
...
default. Set `CARGO_TARGET_DIR` to choose the build output directory; otherwise
the harness uses its local `target/` directory.

```bash
crates/cellule-ltx/perf/run.sh
```
...
```bash
CARGO_TARGET_DIR=/path/to/build-output \
  cargo run --release \
  --manifest-path crates/cellule-ltx/perf/cellule/Cargo.toml -- \
