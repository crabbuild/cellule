...
pub(crate) fn decode_index_entry(entry: &[u8]) -> Result<IndexEntry> {
    Ok(IndexEntry {
        page: u32::from_be_bytes(array(entry.get(..4).ok_or(LtxError::LTXCorrupted)?)?),
        offset: u64::from_be_bytes(array(entry.get(4..12).ok_or(LtxError::LTXCorrupted)?)?),
        size: u64::from_be_bytes(array(entry.get(12..20).ok_or(LtxError::LTXCorrupted)?)?),
        hash: array(entry.get(20..52).ok_or(LtxError::LTXCorrupted)?)?,
        checksum: u64::from_be_bytes(array(entry.get(52..60).ok_or(LtxError::LTXCorrupted)?)?),
    })
}
...
pub(crate) fn decode_frame(frame: &[u8], page_size: u32, pgno: u32) -> Result<Vec<u8>> {
    let prefix = frame.get(..FRAME_PREFIX).ok_or(LtxError::LTXCorrupted)?;
    ...
    let n = lz4_flex::block::decompress_into(&frame[FRAME_PREFIX..], &mut data)
        .map_err(|e| LtxError::Other(Box::new(e)))?;
    if n != data.len() {
        return Err(LtxError::LTXCorrupted);
    }
    Ok(data)
