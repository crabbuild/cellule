...
impl Header {
    fn parse(bytes: &[u8]) -> Result<Self> {
        let prefix = bytes.get(..HEADER_BYTES).ok_or(LtxError::LTXCorrupted)?;
        if &prefix[..8] != MAGIC
    ...
    while let Some((digest, remaining, expected)) = pending.pop() {
        ...
        if header.kind == 0 {
            ...
            for entry in entries {
                let expected_page = previous_page.checked_add(1).ok_or(LtxError::LTXCorrupted)?;
                let lock = crate::ltx::lock_pgno(verification.page_size);
...
pub(super) async fn lookup(
    ...
    loop {
        ...
        height = height.checked_sub(1).ok_or(LtxError::LTXCorrupted)?;
    }
}
...
pub(super) async fn lookup_run(
    ...
    let last = if (first..=requested_last).contains(&lock) {
        lock.checked_sub(1).ok_or(LtxError::LTXCorrupted)?
    } else {
        requested_last
    };
    if last < first {
    ...
    let expected_entries = usize::try_from(last - first + 1).map_err(|_| LtxError::LTXCorrupted)?;
    if entries.len() != expected_entries {
        return Err(LtxError::LTXCorrupted);
    }
    for (index, entry) in entries.iter().enumerate() {
        let expected_page = first
            .checked_add(u32::try_from(index).map_err(|_| LtxError::LTXCorrupted)?)
            .ok_or(LtxError::LTXCorrupted)?;
...
fn verify_leaf(
    ...
    for index in 0..header.entries as usize {
        ...
        let extent = extents.get(&object).ok_or(LtxError::LTXCorrupted)?;
