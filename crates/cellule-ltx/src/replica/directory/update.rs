...
pub(super) async fn run(
    ...
    while let Some((leaf, _)) = outside.peek() {
        ...
        let node_index = u32::try_from(node_index).map_err(|_| LtxError::LTXCorrupted)?;
...
fn changes_for_node(changes: &ChangedLeaves, index: u32, level: u32) -> Result<ChangedLeaves> {
    ...
    let first = u64::from(index)
        .checked_mul(span)
        .ok_or(LtxError::LTXCorrupted)?;
    let end = first.checked_add(span).ok_or(LtxError::LTXCorrupted)?;
    let first = u32::try_from(first).map_err(|_| LtxError::LTXCorrupted)?;
    let end = u32::try_from(end).map_err(|_| LtxError::LTXCorrupted)?;
    Ok(changes
        .range(first..end)
        .map(|(leaf, entries)| (*leaf, entries.clone()))
        .collect())
...
fn node_index_for_leaf(leaf: u32, level: u32) -> Result<u32> {
    let span = node_span(level)?;
    u32::try_from(u64::from(leaf) / span).map_err(|_| LtxError::LTXCorrupted)
}
