...
use crate::{Host, LtxError, Position, Result};
...
pub(super) async fn run(database: &CellPagedDatabase, destination: &Path) -> Result<Position> {
    ...
    let mut downloads = stream::iter(windows.map(move |(first, count)| {
        ...
        async move {
            if first == lock {
                Ok::<_, LtxError>((first, count, DownloadedWindow::Lock))
            } else {
                Ok((
                    first,
                    count,
...
impl RestoreScratch {
    fn create(
        ...
        let filename = destination
            .file_name()
            .ok_or(LtxError::InvalidState("missing restore filename"))?;
