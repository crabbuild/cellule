...
impl<R: Read> Decoder<R> {
    ...
    pub(crate) fn artifact(&self) -> Result<(u64, [u8; 32])> {
        if self.state != DecoderState::Closed {
            return Err(LtxError::InvalidState("LTX decoder is not closed"));
        }
        Ok((
            self.reader.bytes,
...
impl EncoderIndex {
    ...
    fn read_exact_at(&mut self, offset: u64, length: usize) -> Result<Vec<u8>> {
        match self {
            Self::Memory(index) => {
                let start = usize::try_from(offset).map_err(|_| LtxError::LTXCorrupted)?;
                let end = start.checked_add(length).ok_or(LtxError::LTXCorrupted)?;
                Ok(index
                    .get(start..end)
...
pub(crate) fn decode_page_index(bytes: &[u8]) -> Result<Vec<(u32, u64, u64)>> {
    ...
    loop {
        ...
        entries.push((
            u32::try_from(page_number).map_err(|_| LtxError::LTXCorrupted)?,
            offset,
            size,
        ));
    }
...
fn read_uvarint(bytes: &[u8], position: &mut usize) -> Result<u64> {
    ...
    let mut shift = 0u32;
    loop {
        let byte = *bytes.get(*position).ok_or(LtxError::LTXCorrupted)?;
        *position += 1;
        if byte < 0x80 {
