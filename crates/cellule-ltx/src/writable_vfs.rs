...
use crate::{LtxError, Result, paged_io::Io};
...
impl Registration {
    pub(crate) fn new(database: crate::paged_io::Database, path: &Path) -> Result<Self> {
        ...
        let path = host.filesystem.canonicalize(parent)?.join(
            path.file_name()
                .ok_or(LtxError::InvalidState("missing database filename"))?,
        );
        if path.to_str().is_none() {
            return Err(LtxError::InvalidState("database path must be UTF-8"));
        }
        ...
        let mut registry = views()
            .lock()
            .map_err(|_| LtxError::InvalidState("sparse registry poisoned"))?;
    ...
    pub(crate) fn hydration(&self) -> Result<Hydration> {
        let state = self
            ...
            .state
            .lock()
            .map_err(|_| LtxError::InvalidState("sparse state poisoned"))?;
        Ok(Hydration {
    ...
    pub(crate) fn take_error(&self) -> Option<LtxError> {
...
fn page_range(amount: c_int, offset: i64, page_size: u32) -> Result<(u32, u32)> {
    ...
    Ok((
        u32::try_from(offset / i64::from(page_size) + 1).map_err(|_| LtxError::LTXCorrupted)?,
        u32::try_from(end / i64::from(page_size) + 1).map_err(|_| LtxError::LTXCorrupted)?,
    ))
}
...
fn register_for(base: Option<&str>) -> Result<&'static str> {
    ...
    let mut registrations = REGISTRATIONS
        ...
        .lock()
        .map_err(|_| LtxError::InvalidState("sparse VFS registry poisoned"))?;
    let key = base.map(str::to_owned);
    if let Some(name) = registrations.get(&key) {
        return Ok(name);
    }
    let name = match base {
        None => "cellule-ltx-writable-v1".to_owned(),
        Some(_) => format!("cellule-ltx-writable-v1-{}", registrations.len()),
    };
    let c_name =
        CString::new(name.as_str()).map_err(|_| LtxError::InvalidState("invalid VFS name"))?;
    let c_base = base
        .map(CString::new)
        .transpose()
