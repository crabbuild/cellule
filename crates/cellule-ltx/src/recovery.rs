...
use crate::{
    Limits, LocalSegment, LtxError, Position, Result, SegmentInfo, ltx, pages::PageChecksums,
};
...
impl CompactionScratch {
    fn create(
        ...
        let filename = destination
            .file_name()
            .ok_or(LtxError::InvalidState("missing compaction filename"))?;
        static NEXT: std::sync::atomic::AtomicU64 = std::sync::atomic::AtomicU64::new(1);
        for _ in 0..16 {
            let mut scratch_name = filename.to_owned();
            scratch_name.push(format!(
                ".tmp-cellule-ltx-compaction-{}-{}",
                std::process::id(),
