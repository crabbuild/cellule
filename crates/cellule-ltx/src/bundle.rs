...
use bytes::Bytes;
use cellule_store::{MultipartUploadSource, StorageError};
...
impl BundleBuilder {
    ...
    pub fn push(&mut self, entry: BundleEntry) -> Result<()> {
        if self.poisoned {
            return Err(LtxError::InvalidState("bundle builder is poisoned"));
        }
...
impl Bundle {
    ...
    pub fn decode_bytes(bytes: Bytes, limits: Limits) -> Result<Self> {
        ...
        for row in &rows {
            ...
            let end = start
                .checked_add(
                    usize::try_from(row.info.size_bytes).map_err(|_| LtxError::LTXCorrupted)?,
                )
    ...
    pub fn detach_file(self) -> Result<PathBuf> {
        ...
        let mut file = Arc::try_unwrap(file)
            .map_err(|_| LtxError::InvalidState("bundle file is still referenced"))?;
...
fn decode_footer(bytes: &[u8], limits: Limits) -> Result<(Vec<BundleRow>, u64)> {
    let trailer = bytes.len().checked_sub(8).ok_or(LtxError::LTXCorrupted)?;
...
fn decode_file_rows(path: &Path, limits: Limits) -> Result<(Vec<BundleRow>, u64)> {
    ...
    for row in &rows {
        let size = usize::try_from(row.info.size_bytes).map_err(|_| LtxError::LTXCorrupted)?;
