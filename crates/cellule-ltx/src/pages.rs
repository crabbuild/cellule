...
use crate::{CHECKSUM_FLAG, LtxError, Result, ltx};
...
impl PageChecksums {
    ...
    fn remove_file_suffix(
        ...
        let file = file.ok_or(LtxError::InvalidState("checksum file not open"))?;
        let mut page = commit.checked_add(1).ok_or(LtxError::LTXCorrupted)?;
        ...
        while page <= file_end {
            ...
            for checksum in bytes.as_chunks::<8>().0 {
                ...
                self.checksum = CHECKSUM_FLAG | (self.checksum ^ old);
                page = page.checked_add(1).ok_or(LtxError::LTXCorrupted)?;
            }
        }
    ...
    fn value(&self, page: u32, _file: Option<&mut crate::HostFile>) -> Result<u64> {
        ...
        let checksum = match &self.base {
            ChecksumBase::Memory(pages) => {
                *pages.get(page as usize - 1).ok_or(LtxError::LTXCorrupted)?
            }
            #[cfg(feature = "replica")]
            ChecksumBase::File(_) => {
                let file = _file.ok_or(LtxError::InvalidState("checksum file not open"))?;
                let bytes = file.read_exact_at(u64::from(page - 1) * 8, 8)?;
                u64::from_be_bytes(bytes.try_into().map_err(|_| LtxError::LTXCorrupted)?)
            }
        };
