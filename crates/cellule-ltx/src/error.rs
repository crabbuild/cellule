...
mod tests {
    ...
    fn only_cell_graph_admission_limits_are_compaction_retryable() {
        assert!(LtxError::Limit("Cell root segments").is_cell_graph_limit());
        assert!(LtxError::Limit("Cell root bytes").is_cell_graph_limit());
        assert!(!LtxError::Limit("LTX file bytes").is_cell_graph_limit());
        assert!(!LtxError::ChecksumMismatch.is_cell_graph_limit());
    }
}
