...
impl CaptureEngine {
    ...
    pub fn open_with_host(
        ...
        let conn = open(&path).map_err(LtxError::Sqlite)?;
        ...
        let rtx_conn = open(&path).map_err(LtxError::Sqlite)?;
        rtx_conn
    ...
    fn sync_impl(&mut self, required: Option<crate::commit::WalCut>) -> Result<()> {
        ...
        if let Some(required) = required {
            let (_, header) = self.last_l0_header.as_ref().ok_or(LtxError::LTXCorrupted)?;
    ...
    pub fn snapshot_to_writer<W: std::io::Write>(&mut self, w: &mut W) -> Result<Pos> {
        ...
        let mut rd = WalReader::new(&wal.bytes).map_err(LtxError::from)?;
        let (page_map, max_offset, wal_commit) = rd.page_map().map_err(LtxError::from)?;
        if wal_commit > 0 {
            commit = wal_commit;
        }
...
fn rollback(conn: &Connection) -> Result<()> {
    ...
    conn.execute_batch("ROLLBACK").map_err(LtxError::Sqlite)
}
