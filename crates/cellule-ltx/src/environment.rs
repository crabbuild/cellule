...
impl DiskBudget {
    ...
    pub fn install_admission(&self, admission: Arc<dyn DiskBudgetAdmission>) -> crate::Result<()> {
        let mut current = self
            ...
            .admissions
            .lock()
            .map_err(|_| crate::LtxError::InvalidState("disk admission lock poisoned"))?;
        ...
        let result = current.last().map_or_else(
            || {
                Err(crate::LtxError::InvalidState(
    ...
    fn add(&self, bytes: u64) -> crate::Result<()> {
        self.inner
            ...
            .map(|_| ())
            .map_err(|_| crate::LtxError::Limit("local disk bytes"))
    }
...
impl DiskReservation {
    ...
    pub fn try_grow(&self, bytes: u64) -> crate::Result<()> {
        ...
        let next = held
            .checked_add(bytes)
            .ok_or(crate::LtxError::Limit("local disk bytes"))?;
...
impl Host {
    ...
    pub(crate) async fn for_scratch(&self, bytes: u64) -> crate::Result<Self> {
        ...
        let units = bytes
            .checked_add(MIB - 1)
            .ok_or(crate::LtxError::Limit("scratch disk bytes"))?
            / MIB;
        let units =
            u32::try_from(units).map_err(|_| crate::LtxError::Limit("scratch disk bytes"))?;
        if units == 0 || units > self.scratch_capacity {
            return Err(crate::LtxError::Limit("scratch disk bytes"));
        }
        let mut host = self.clone();
    ...
    pub(crate) async fn run<T: Send + 'static>(
        ...
        self.executor.dispatch(Box::new(move || {
            ...
            let result = std::panic::catch_unwind(std::panic::AssertUnwindSafe(operation))
                .map_err(|_| crate::LtxError::InvalidState("host job panicked"));
...
mod tests {
    ...
    fn disk_budget_reservations_resize_and_release_exact_bytes() {
        ...
        assert!(matches!(
            ...
            Err(crate::LtxError::Limit("local disk bytes"))
        ));
