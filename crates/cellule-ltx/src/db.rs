...
impl SnapshotScratch {
    fn create(
        ...
        let filename = destination
            .file_name()
            .ok_or(LtxError::InvalidState("missing snapshot filename"))?;
...
pub(crate) fn read_main(connection: &Connection, offset: u64, size: usize) -> Result<Vec<u8>> {
    ...
    let offset = i64::try_from(offset).map_err(|_| LtxError::LTXCorrupted)?;
    let size = i32::try_from(size).map_err(|_| LtxError::LTXCorrupted)?;
    ...
    let rc = unsafe {
        ...
        if rc != ffi::SQLITE_OK || file.is_null() || (*file).pMethods.is_null() {
            return Err(LtxError::InvalidState("SQLite main file unavailable"));
        }
        let read = (*(*file).pMethods)
            .xRead
            .ok_or(LtxError::InvalidState("SQLite main file cannot read"))?;
