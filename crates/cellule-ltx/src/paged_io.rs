...
use crate::{CellWritableDatabase, LtxError, Result};
use std::{
    cell::Cell,
...
impl Driver {
    fn new(host: &crate::Host) -> Result<Arc<Self>> {
        ...
        started.recv().map_err(|e| LtxError::Other(Box::new(e)))??;
        Ok(driver)
    }
}
...
impl Io {
    pub(crate) fn new(database: Database) -> Result<Self> {
        ...
        let mut slot = host
            .paged_driver
            .lock()
            .map_err(|_| LtxError::InvalidState("paged driver slot poisoned"))?;
    ...
    pub(crate) fn page(&self, page: u32) -> Result<Vec<u8>> {
        ...
        let _gate = self
            .gate
            .lock()
            .map_err(|_| LtxError::InvalidState("paged request gate poisoned"))?;
        ...
        self.driver
            .sender
            .as_ref()
            .ok_or(LtxError::InvalidState("paged I/O closed"))?
            .try_send(request)
...
fn receive(response: mpsc::Receiver<Result<Vec<u8>>>, deadline: Instant) -> Result<Vec<u8>> {
    match response.recv_timeout(deadline.saturating_duration_since(Instant::now())) {
        Ok(result) => result,
        Err(mpsc::RecvTimeoutError::Timeout) => Err(LtxError::Deadline),
        Err(error @ mpsc::RecvTimeoutError::Disconnected) => Err(LtxError::Other(Box::new(error))),
    }
}
...
mod tests {
    ...
    fn blocking_page_wait_stops_at_its_deadline() {
        ...
        drop(reply);
        assert!(matches!(result, Err(LtxError::Deadline)));
