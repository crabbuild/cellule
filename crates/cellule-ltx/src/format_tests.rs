...
use crate::{
    CHECKSUM_FLAG, Limits, LocalSegment, LtxError, Position, SegmentInfo, VerifiedPlan, ltx,
};
...
fn exact_restore_rejects_checksum_disabled_file() {
    ...
    assert!(matches!(result, Err(LtxError::LTXCorrupted)));
}
...
fn altered_delta_predecessor_or_post_state_is_rejected_with_valid_file_crc() {
    ...
    for (i, bad_pre) in [true, false].into_iter().enumerate() {
        ...
        assert!(matches!(result, Err(LtxError::ChecksumMismatch)));
    }
}
