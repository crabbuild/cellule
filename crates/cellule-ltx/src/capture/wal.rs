...
impl CaptureEngine {
    ...
    pub(super) fn sync_inner(&mut self, mut info: SyncInfo) -> Result<bool> {
        ...
        let tx_id = Txid(pos.txid.0.checked_add(1).ok_or(LtxError::TxNotAvailable)?);
        ...
        let mut rd = if info.offset == WAL_HEADER_SIZE as i64 {
            WalReader::new(&wal.bytes).map_err(LtxError::from)?
        } else {
            wal.reader_at(info.offset, info.salt1, info.salt2)
        ...
        let page_map_result = rd.page_map().map_err(LtxError::from);
    ...
    pub(super) fn read_db_page(&self, pgno: u32) -> Result<Vec<u8>> {
        let offset = u64::from(pgno.checked_sub(1).ok_or(LtxError::LTXCorrupted)?)
