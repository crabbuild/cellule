...
```sh
export CELLULE_LTX_TEST_BUCKET=cellule-ltx-examples
...
Mutable owner/epoch/root publication remains a `cellule-runtime` authority
operation. `cellule-ltx` prepares immutable bytes and never acknowledges an HTTP
