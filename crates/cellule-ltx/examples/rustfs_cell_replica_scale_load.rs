...
use cellule_ltx::{CellReplica, CellStorageLayout, Db, Limits, LtxError, RootRef};
...
#[tokio::main]
async fn main() -> cellule_ltx::Result<()> {
    ...
    let mut prepared: Option<cellule_ltx::PreparedRoot> = None;
    ...
    while written < target_bytes {
        ...
        let rows = i64::try_from(batch_bytes / ROW_BYTES as u64)
            .map_err(|_| LtxError::InvalidState("Cell scale row count"))?;
        let (next_writer, capture) = tokio::task::spawn_blocking(move || {
            ...
            Ok::<_, cellule_ltx::LtxError>((writer, capture))
        })
        .await
        .map_err(|_| LtxError::InvalidState("Cell scale writer stopped"))??;
        writer = next_writer;
        ...
        let (next_writer, _) = tokio::task::spawn_blocking(move || {
            ...
            Ok::<_, cellule_ltx::LtxError>((writer, removed))
        })
        .await
        .map_err(|_| LtxError::InvalidState("Cell scale prune stopped"))??;
        writer = next_writer;
        written = written
            .checked_add(batch_bytes)
            .ok_or(LtxError::Limit("Cell scale bytes"))?;
        sequence = sequence
            .checked_add(1)
            .ok_or(LtxError::Limit("Cell scale sequence"))?;
    ...
    let prepared = prepared.ok_or(LtxError::InvalidState("Cell scale produced no root"))?;
    let root = root.ok_or(LtxError::InvalidState("Cell scale produced no root"))?;
...
fn workload_root() -> cellule_ltx::Result<PathBuf> {
    let root = std::env::var_os("CELLULE_LTX_WORKLOAD_ROOT").ok_or(LtxError::InvalidState(
        "CELLULE_LTX_WORKLOAD_ROOT is required",
    ))?;
...
fn stream_digest(path: &Path) -> cellule_ltx::Result<(u64, [u8; 32])> {
    ...
    loop {
        ...
        length = length
            ...
            .ok_or(LtxError::Limit("Cell scale checksum length"))?;
    }
    Ok((length, *hasher.finalize().as_bytes()))
}

fn remove_sqlite_artifacts(database: &Path) -> cellule_ltx::Result<()> {
