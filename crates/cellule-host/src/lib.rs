...
/// owner that constructs and installs [`cellule_runtime::NodeDurability`].
...
impl CellNodeFacility {
    ...
    pub fn new<F, Fut>(name: &'static str, drain: F) -> cellule_runtime::Result<Self>
    where
...
impl CellNodeTaskGroup {
    ...
    fn ensure_accepting_tasks(&self) -> cellule_runtime::Result<()> {
...
impl CellNodeBuilder {
    ...
    pub fn build(self) -> cellule_runtime::Result<CellNode> {
        let CellNodeParts {
            application,
            pool,
            replica_host,
    ...
    fn required_parts(self) -> cellule_runtime::Result<CellNodeParts> {
        let pool = self
            .pool
...
impl CellNode {
    ...
    pub fn install_telemetry(
        &self,
        telemetry: Arc<dyn cellule_runtime::CellTelemetry>,
    ) -> cellule_runtime::Result<()> {
    ...
    fn require_components_present(&self) -> cellule_runtime::Result<()> {
        let required = self
            .required_components
            .lock()
            .map_err(|_| Error::Control("CellNode required-component lock poisoned"))?
    ...
    fn remove_facility(&self, name: &'static str) -> cellule_runtime::Result<()> {
    ...
    fn require_task_group(&self) -> cellule_runtime::Result<()> {
        let installed = self
            .task_group
            .lock()
            .map_err(|_| Error::Control("CellNode task group lock poisoned"))?;
...
mod tests {
        ...
        fn descriptor(&self) -> &'static ModuleDescriptor {
            static DESCRIPTOR: ModuleDescriptor = ModuleDescriptor {
                ...
                namespaces: &[NamespaceDescriptor {
                    id: cellule_runtime::NamespaceId::from_bytes([2; 16]),
                    name: "host-test",
    ...
    fn application() -> Arc<CompiledApplication> {
        let mut builder = cellule_app::ApplicationBuilder::new(
            "host-test",
            BuildDescriptor {
        ...
        builder
            .cell_type(
                cellule_app::CellType::new(
                    "host-test",
                    "host-test",
                    cellule_runtime::NamespaceId::from_bytes([2; 16]),
                    CatalogRole::Sql,
                    1,
                )
                .unwrap(),
