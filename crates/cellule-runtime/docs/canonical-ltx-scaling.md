...
Production server code must not call `cellule-ltx` directly. Test fixtures may use
...
Replica telemetry crosses into `cellule-runtime` only as the closed enums
...
ledger admission on `cellule_ltx::DiskBudget`, imports existing local bytes, and
...
`coordination` module inside `cellule-runtime` that accepts plain immutable
...
Add a TLA+ model under `crates/cellule-runtime/model/` for two Cells, up to
...
private to `cellule-ltx`; callers continue to use `CellReplica::prepare`,
...
The server may supply `cellule-ltx::Host` with a caller-owned local directory-node
...
| --- | --- | --- | --- |
| 1 | Correct ownership docs and add server dependency guard | None | Architecture check rejects production `cellule_ltx` imports |
