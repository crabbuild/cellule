...
`cellule-ltx` already supplies capture, checksum-bearing segments, bundle extents,
...
`cellule-ltx` to verify and materialize a successor root, then CASes that exact
...
struct NodeLogFrameV1 {
    ...
    cell_epoch: u64,
    commit_sequence: u64,
    segment: cellule_ltx::SegmentInfo,
    body_len: u64,
    body_blake3: [u8; 32],
    body: Bytes,
}
...
| 2 | `cellule-ltx` verified frame and recovered-overlay API | Golden, corruption, cross-epoch, and exact-root tests |
