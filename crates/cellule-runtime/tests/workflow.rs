...
fn workflow_event_counter_tracks_history_and_reports_drift() {
    ...
    cellule_runtime::verify_workflow_event_count(&transaction).unwrap();
    transaction
        .execute(
        ...
        .unwrap();
    assert!(cellule_runtime::verify_workflow_event_count(&transaction).is_err());
}
...
fn terminal_transition_inserts_effect_with_cell_command_identity() {
    ...
    assert_eq!(
        stored.0,
        cellule_runtime::effect_id(source.cell_id(), IncarnationId::from_bytes([2; 16]), 1, 0,)
            .to_vec()
    );
