...
use cellule_ltx::{CellReplica, Db, Limits};
use cellule_runtime::{
...
fn fixture_with_store(store: Store) -> Fixture {
    ...
    let mut connection = cellule_ltx::rusqlite::Connection::open(&database).unwrap();
...
async fn prepared_root_becomes_one_valid_control_successor() {
    ...
    let wrong_root = cellule_ltx::RootRef {
        digest: [11; 32],
        ..prepared.root()
    };
...
async fn business_rejection_rolls_back_domain_writes_and_publishes_the_outcome() {
    ...
    let connection = cellule_ltx::rusqlite::Connection::open(database).unwrap();
    assert_eq!(
        connection
