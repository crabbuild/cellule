...
impl MaintenanceModule for SourceQueue {
    ...
    const TICK_COMMAND_ID: u32 = 4;
    const QUEUE_DEAD_LETTER: Option<cellule_runtime::QueueDeadLetterTarget> =
        Some(cellule_runtime::QueueDeadLetterTarget::new(
            DEAD_LETTER_MODULE,
...
fn queue_registry() -> Arc<cellule_runtime::Registry> {
...
impl QueueTokenSource for Tokens {
    fn next_token(&mut self) -> cellule_runtime::Result<[u8; 16]> {
        self.0 = self
            .0
            .checked_add(1)
            .ok_or(cellule_runtime::Error::Command("test queue token overflow"))?;
        Ok([self.0; 16])
    }
}

fn connection() -> cellule_ltx::rusqlite::Connection {
