...
impl NodeLogTransport for TestNodeTransport {
    fn append<'a>(
        ...
        Box::pin(async move {
            ...
            let last = request.frames.last().ok_or(cellule_runtime::Error::Node(
                "test node-log append is empty",
            ))?;
            let frame = cellule_ltx::inspect_node_frame(last.clone(), Limits::default())?;
            Ok(FollowerReceipt {
                base_sequence: 1,
    ...
    fn seal<'a>(
        ...
        Box::pin(async { Err(cellule_runtime::Error::Node("unused test seal")) })
    }
    ...
    fn tail<'a>(
        ...
        Box::pin(async { Err(cellule_runtime::Error::Node("unused test tail")) })
    }
...
struct LostAckFollowerTransport {
    inner: cellule_runtime::LocalFollowerTransport,
...
async fn fence_session(
    ...
    session: SessionId,
    claimant: SessionId,
) -> cellule_runtime::FencedNodeSession {
    let fleet = Digest::from_bytes([90; 32]);
    let image = Digest::from_bytes([91; 32]);
    let release = Digest::from_bytes([92; 32]);
    let directory = cellule_runtime::NodeDirectory::new(layout.clone(), fleet, image, release);
    let key = ed25519_dalek::SigningKey::from_bytes(&[93; 32]);
    directory
        .create(
            cellule_runtime::NodeAdvertisement::sign(
                ...
                cellule_runtime::NodeCapacity {
                    free_memory_bytes: 1,
                    free_disk_bytes: 1,
                    job_credits: 1,
                    ..cellule_runtime::NodeCapacity::default()
                },
            )
            .unwrap(),
            1,
    ...
    directory
        .create(
            cellule_runtime::NodeAdvertisement::sign(
                cellule_runtime::NodeId::from_bytes(*claimant.as_bytes()),
                claimant,
...
async fn fence_log_session(
    ...
    let key = ed25519_dalek::SigningKey::from_bytes(&[93; 32]);
    let signed =
        |session: cellule_runtime::SessionId, endpoint: &str, issued_at_ms, expires_at_ms| {
            cellule_runtime::NodeAdvertisement::sign(
                cellule_runtime::NodeId::from_bytes(*session.as_bytes()),
                session,
                endpoint.into(),
...
async fn recovery_inventory_reads_catalog_heads_concurrently() {
    ...
    let inventory = tokio::time::timeout(
        ...
        cellule_runtime::recoverable_cells(
            &catalog,
            &authority,
...
async fn activate(fixture: &Fixture, node_bytes: usize) -> cellule_runtime::CellHandle {
...
async fn activate_runtime(
    fixture: &Fixture,
    node_bytes: usize,
) -> (CellRuntime, cellule_runtime::CellHandle, SqlWorkerPool) {
...
async fn lost_ack_suffix_recovers_an_ambiguous_command_without_reexecution() {
    ...
    let transport: Arc<dyn NodeLogTransport> = Arc::new(LostAckFollowerTransport {
        inner: cellule_runtime::LocalFollowerTransport::new(member, follower_store.clone()),
    ...
    let recovery = cellule_runtime::NodeLogRecovery::from_fenced(
        Arc::clone(&transport),
        &fenced,
        Limits::default(),
    )
    .unwrap();
    let manifests =
        cellule_runtime::RecoveryManifestStore::new(fixture.layout.clone(), Limits::default());
    let coordinator = cellule_runtime::RecoveryCoordinator::new(recovery, manifests.clone());
    let inventory = cellule_runtime::recoverable_cells(&catalog, &authority, leader, 10)
        .await
        .unwrap();
...
async fn runtime_stats_follow_active_cell_lifecycle() {
    ...
    assert_eq!(
        ...
        cellule_runtime::ACTIVE_CELL_NATIVE_BYTES as usize
    );
    assert_eq!(
        runtime.stats().resident_capacity_bytes(),
        10 * cellule_runtime::ACTIVE_CELL_NATIVE_BYTES as usize
    );
    assert_eq!(
...
async fn churn_evicts_idle_cells_and_restores_exact_roots() {
    ...
    let catalog = cellule_runtime::CellCatalog::new(
...
async fn mixed_primitive_inventory_churn(store: Store, prefix: Path) {
    ...
    let queue_proof =
        cellule_runtime::CellCatalog::new(queue.layout.clone(), queue.target.tenant())
    ...
    assert_eq!(
        cellule_runtime::CellAuthority::new(queue.layout.clone())
            .load(queue.target.cell_id())
            .await
            .unwrap()
            .unwrap()
...
async fn wait_for_persisted_work(
    handle: &cellule_runtime::CellHandle,
    role: CatalogRole,
    blocker: &'static str,
) {
    tokio::time::timeout(std::time::Duration::from_secs(3), async {
    ...
    assert!(
        runtime
            .takeover_restored(
                ...
                cellule_runtime::RecoveryManifestStore::new(
                    fixture.layout.clone(),
                    Limits::default(),
                ),
                missing_parent,
...
async fn recover_retained_tail(rooted: bool) {
    ...
    let transport: Arc<dyn cellule_runtime::NodeLogTransport> =
        Arc::new(cellule_runtime::LocalFollowerTransport::new(
            cellule_runtime::NodeId::from_bytes(*follower.as_bytes()),
            follower_store,
        ));
    transport
        .append(
            cellule_runtime::NodeId::from_bytes(*follower.as_bytes()),
            cellule_runtime::AppendRequest {
    ...
    if rooted {
        ...
        assert_eq!(
            ...
            cellule_runtime::NodeLogPhase::Sealed
        );
        return;
    }
    ...
    let resumed_recovery =
        cellule_runtime::NodeLogRecovery::from_fenced(transport, &fenced, Limits::default())
            .unwrap();
    let resumed = cellule_runtime::RecoveryCoordinator::new(resumed_recovery, manifests.clone());
    let directory = cellule_runtime::NodeDirectory::new(
        fixture.layout.clone(),
        Digest::from_bytes([90; 32]),
        Digest::from_bytes([91; 32]),
    ...
    let completed = resumed
        .recover_and_seal(
            &directory,
            fenced.clone(),
            vec![cellule_runtime::RecoveryCell {
...
async fn query_waits_for_preceding_publication_and_cannot_write() {
    ...
    assert!(matches!(
        handle
            ...
            .await,
        Err(cellule_runtime::Error::Sqlite(_))
    ));
    assert_eq!(
        handle
            .query(64, 64, |connection| {
        ...
        runtime
            .bootstrap(
                ...
                |transaction| {
                    ...
                    Err(cellule_runtime::Error::Command("migration rejected"))
                },
            )
            .await,
        Err(cellule_runtime::Error::Command("migration rejected"))
...
async fn panicking_bootstrap_keeps_worker_alive_and_releases_cell_capacity() {
    ...
    assert!(matches!(
        runtime
            ...
            .await,
        Err(cellule_runtime::Error::NativePanic)
    ));
    let current = authority
        .load(fixture.target.cell_id())
        .await
...
async fn source_loss_takeover(store: Store, prefix: Path) {
    ...
    .unwrap();
    let catalog = cellule_runtime::CellCatalog::new(layout.clone(), target.tenant());
    let proof = catalog
        .provision(
            CatalogEntry::new(
                &target,
