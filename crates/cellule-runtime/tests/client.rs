...
impl Command for EmitEffectComment {
    ...
    fn execute(
        ...
        let expires_at_ms = context
            .now_ms()
            .checked_add(60_000)
            .ok_or(cellule_runtime::Error::Command("effect expiry overflow"))?;
    ...
    fn execute(
        ...
        match results[0].rows.first().and_then(|row| row.first()) {
            Some(SqlValue::Integer(count)) => u64::try_from(*count)
                .map_err(|_| cellule_runtime::Error::Command("negative comment count")),
...
struct Fixture {
    ...
    target: CellTarget,
    handle: Option<cellule_runtime::CellHandle>,
    registry: Arc<Registry>,
}

impl Fixture {
    fn handle(&self) -> &cellule_runtime::CellHandle {
        self.handle.as_ref().expect("fixture handle is present")
    }

    fn take_handle(&mut self) -> cellule_runtime::CellHandle {
...
impl PeerCellResolver for LocalResolver {
    fn resolve(
        ...
        Box<
            dyn Future<Output = cellule_runtime::Result<cellule_runtime::CellHandle>>
                + Send
                + 'static,
        >,
    > {
...
impl PeerAuthorizer for RepositoryAuthorizer {
    fn authorize(&self, request: &VerifiedPeerRequest) -> cellule_runtime::Result<()> {
        if request.permits("repository.issue.create") {
            ...
            Err(cellule_runtime::Error::PeerAuthorization(
    ...
    fn send(
        ...
        Box::pin(async move {
            let now_ms = i64::try_from(
                std::time::SystemTime::now()
                    .duration_since(UNIX_EPOCH)
                    .map_err(|_| cellule_runtime::Error::Peer("test clock"))?
                    .as_millis(),
            )
            .map_err(|_| cellule_runtime::Error::Peer("test clock overflow"))?;
            let verified = verifier.verify(&request, now_ms)?;
            if verified.target() != &target {
                return Err(cellule_runtime::Error::Peer("round trip target changed"));
            }
...
async fn authenticated_effect_delivery_publishes_once_and_resolves_from_inbox() {
    ...
    .unwrap();
    let source_cell = cellule_runtime::CellId::from_bytes([21; 32]);
...
async fn typed_client_rejects_conflicting_identity_receipt_and_module_before_execution() {
    ...
    assert!(matches!(
        ...
        Err(InvocationError::NotStarted(
            cellule_runtime::Error::RequestConflict
        ))
    ));
    assert!(matches!(
        client
            ...
            .await,
        Err(InvocationError::NotStarted(
            cellule_runtime::Error::Command("minimum receipt does not match Cell")
        ))
    ));
    assert!(matches!(
        client
            ...
            .await,
        Err(InvocationError::NotStarted(
            cellule_runtime::Error::Registry("operation module does not own namespace")
        ))
    ));
    let now_ms = i64::try_from(
        std::time::SystemTime::now()
    ...
    assert!(matches!(
        client
            ...
            .await,
        Err(InvocationError::NotStarted(
            cellule_runtime::Error::Command("invalid mutation identity lifetime")
        ))
    ));
...
async fn command_effects_require_declared_same_application_targets() {
    ...
    assert!(matches!(
        result,
        Err(InvocationError::NotStarted(
            cellule_runtime::Error::Command("effect target is not declared")
        ))
    ));
    assert_eq!(
        client
...
fn command_digest_is_canonical_and_binds_incarnation_identity_and_input() {
    let description = CellDescription {
        cell: cellule_runtime::CellId::from_bytes([1; 32]),
