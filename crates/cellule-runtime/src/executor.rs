...
pub struct PendingCommit {
    ...
    cuts: CaptureBatch,
    prepared: Option<cellule_ltx::RootRef>,
    durable: bool,
}
...
impl PendingCommit {
    ...
    #[must_use]
    pub fn prepared(&self) -> Option<cellule_ltx::RootRef> {
        self.prepared
    }
...
impl CellExecutor {
    ...
    pub(crate) fn from_restored(
        ...
        if let Err(error) = verification {
            let error = db.take_io_error().map_or_else(
                || match error {
                    cellule_ltx::QueryError::Operation(error) => error,
                    cellule_ltx::QueryError::Sqlite(error) => error.into(),
                    cellule_ltx::QueryError::State(error) => error.into(),
                },
                ltx_error,
            );
            let _ = db.close();
    ...
    pub fn execute(
        ...
        handler: impl FnOnce(&cellule_ltx::rusqlite::Transaction<'_>) -> Result<HandlerOutcome>,
    ...
    pub fn query(
        &mut self,
        max_result_bytes: usize,
        handler: impl FnOnce(&cellule_ltx::rusqlite::Connection) -> Result<Vec<u8>>,
    ) -> Result<Vec<u8>> {
        if self.fenced {
            return Err(Error::Fenced);
        }
        ...
        match result {
            ...
            Err(cellule_ltx::QueryError::Operation(error)) => Err(error),
            Err(cellule_ltx::QueryError::Sqlite(error)) => {
                self.fenced = true;
                Err(error.into())
            }
            Err(cellule_ltx::QueryError::State(error)) => {
                self.fenced = true;
                Err(error.into())
            }
        }
    ...
    pub(crate) fn hydrate_step(&mut self, pages: u32) -> Result<Option<cellule_ltx::Hydration>> {
        if self.fenced {
            return Err(Error::Fenced);
        }
        if pages == 0 {
    ...
    pub(crate) fn hydration(&self) -> Result<Option<cellule_ltx::Hydration>> {
    ...
    pub fn resolve(
        ...
        let existing = match result {
            ...
            Err(cellule_ltx::QueryError::Operation(error)) => return Err(error.into()),
            Err(cellule_ltx::QueryError::Sqlite(error)) => {
                self.fenced = true;
                return Err(error.into());
            }
    ...
    pub fn bind_prepared(&mut self, prepared: &cellule_ltx::PreparedRoot) -> Result<()> {
...
fn admission_error(error: cellule_ltx::LtxError) -> Error {
    match error {
        cellule_ltx::LtxError::Limit("local disk bytes") => Error::Capacity("local disk bytes"),
        error => error.into(),
    }
}
...
fn ltx_error(error: cellule_ltx::LtxError) -> Error {
    match error {
        cellule_ltx::LtxError::Deadline => Error::Deadline,
