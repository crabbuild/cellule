...
impl cellule_ltx::DiskBudgetAdmission for LedgerDiskAdmission {
    fn reconcile(&self, bytes: u64) -> cellule_ltx::Result<()> {
        ...
        ResourceLedger { state }
            ...
            .map_err(|error| cellule_ltx::LtxError::Other(Box::new(error)))?;
        Ok(())
    }
...
impl cellule_ltx::HostResourcePermit for LedgerHostResourcePermit {}

impl cellule_ltx::HostResourceAdmission for LedgerHostResourceAdmission {
    fn reserve(
        &self,
        kind: cellule_ltx::HostResourceKind,
        units: u32,
    ) -> cellule_ltx::Result<Box<dyn cellule_ltx::HostResourcePermit>> {
        let Some(state) = self.state.upgrade() else {
            return Err(cellule_ltx::LtxError::InvalidState("runtime ledger closed"));
        };
        let units = usize::try_from(units)
            .map_err(|_| cellule_ltx::LtxError::Limit("runtime host resource units"))?;
        let cost = match kind {
            cellule_ltx::HostResourceKind::Io => ResourceCost::zero().with_io_slots(units),
            cellule_ltx::HostResourceKind::BlockingJob => {
                ResourceCost::zero().with_blocking_jobs(units)
            }
            cellule_ltx::HostResourceKind::Recovery => {
                ResourceCost::zero().with_recovery_jobs(units)
            }
            cellule_ltx::HostResourceKind::Dirty => ResourceCost::zero().with_dirty_jobs(units),
            cellule_ltx::HostResourceKind::Scratch => {
                ResourceCost::zero().with_scratch_units(units)
            }
        };
...
mod tests {
    ...
    fn ltx_disk_reservations_share_the_runtime_ledger() {
        ...
        let budget = cellule_ltx::DiskBudget::new(20);
    ...
    fn ltx_host_admissions_share_one_runtime_ledger() {
        ...
        let io = admission
            .reserve(cellule_ltx::HostResourceKind::Io, 1)
            .unwrap();
        assert!(
            admission
                .reserve(cellule_ltx::HostResourceKind::Io, 1)
        ...
        let scratch = admission
            .reserve(cellule_ltx::HostResourceKind::Scratch, 2)
            .unwrap();
        assert!(
            admission
                .reserve(cellule_ltx::HostResourceKind::Scratch, 1)
                .is_err()
        );
