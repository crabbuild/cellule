...
pub const ACTIVE_CELL_PAGE_CACHE_BYTES: u64 =
    cellule_ltx::MANAGED_SQLITE_CONNECTIONS * cellule_ltx::MANAGED_CONNECTION_PAGE_CACHE_BYTES;
...
pub(crate) type Initializer = Box<
    dyn for<'connection> FnOnce(&cellule_ltx::rusqlite::Transaction<'connection>) -> Result<()>
        + Send
        + 'static,
>;

pub(crate) type QueryHandler =
    Box<dyn FnOnce(&cellule_ltx::rusqlite::Connection) -> Result<Vec<u8>> + Send + 'static>;

pub(crate) struct BootstrapExecution {
    pub(crate) cuts: cellule_ltx::CaptureBatch,
...
impl SqlWorkerPool {
    ...
    pub(crate) async fn activate_restored(
        &self,
        cell: CellId,
        database: cellule_ltx::CellWritableDatabase,
        destination: PathBuf,
        incarnation: crate::IncarnationId,
        schema: u32,
    ...
    pub(crate) async fn hydrate(
        ...
        pages: u32,
        deadline: Instant,
    ) -> Result<Option<cellule_ltx::Hydration>> {
        let (reply, response) = oneshot::channel();
        self.send_worker_job(
            cell,
    ...
    pub(crate) async fn hydration(&self, cell: CellId) -> Result<Option<cellule_ltx::Hydration>> {
        let (reply, response) = oneshot::channel();
...
enum WorkerCommand {
    ...
    ActivateRestored {
        cell: CellId,
        database: Box<cellule_ltx::CellWritableDatabase>,
        destination: PathBuf,
        incarnation: crate::IncarnationId,
        schema: u32,
        root: cellule_ltx::RootRef,
...
fn run_worker_command(
    ...
    match command {
        ...
        WorkerCommand::Resolve {
            ...
            let result = cellule_ltx::with_paged_io_deadline(deadline, || {
                cells
                    .get_mut(&cell)
                    .ok_or(Error::CellNotActive)
                    .and_then(|cell| {
