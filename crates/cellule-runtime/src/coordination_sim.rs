...
fn replay_requested_seed_from_environment() {
    let Ok(seed) = env::var("CELLULE_COORDINATION_SEED") else {
        return;
    };
    let seed = seed
        .parse::<u64>()
        .expect("CELLULE_COORDINATION_SEED must be an unsigned integer");
    let steps = env::var("CELLULE_COORDINATION_STEPS")
        .map(|value| {
            value
                .parse::<usize>()
                .expect("CELLULE_COORDINATION_STEPS must be an unsigned integer")
        })
        .unwrap_or(128);
    assert!(
        steps <= 4_096,
        "CELLULE_COORDINATION_STEPS exceeds the replay bound"
    );
