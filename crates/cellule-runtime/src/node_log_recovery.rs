...
impl WitnessWriter {
    ...
    fn push(&mut self, frame: &cellule_ltx::VerifiedNodeFrame) -> Result<()> {
...
impl SealedWitness {
    fn reader(&self, limits: cellule_ltx::Limits) -> Result<WitnessReader> {
        Ok(WitnessReader {
...
impl Iterator for WitnessReader {
    type Item = Result<cellule_ltx::VerifiedNodeFrame>;
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        Some(cellule_ltx::inspect_node_frame(encoded.into(), self.limits).map_err(Into::into))
    }
}

enum WitnessCollector {
    Memory(Vec<cellule_ltx::VerifiedNodeFrame>),
    File(WitnessWriter),
}
...
impl WitnessCollector {
    ...
    fn push(&mut self, frames: Vec<cellule_ltx::VerifiedNodeFrame>) -> Result<()> {
        match self {
...
pub struct SealedSession {
    ...
    pub frames: Vec<cellule_ltx::VerifiedNodeFrame>,
    ...
    _reservation: Option<cellule_ltx::DiskReservation>,
}
...
impl SealedSession {
    ...
    pub fn scopes(&self, limits: cellule_ltx::Limits) -> Result<Vec<cellule_ltx::NodeFrameScope>> {
...
fn unique_scopes<I>(scopes: I) -> Result<Vec<cellule_ltx::NodeFrameScope>>
where
    I: IntoIterator<Item = Result<cellule_ltx::NodeFrameScope>>,
{
    let mut unique = BTreeMap::<[u8; 32], cellule_ltx::NodeFrameScope>::new();
    for scope in scopes {
        let scope = scope?;
...
pub async fn recoverable_cells_from_scopes(
    ...
    authority: &CellAuthority,
    owner: SessionId,
    frame_scopes: &[cellule_ltx::NodeFrameScope],
    limit: usize,
) -> Result<Vec<RecoveryCell>> {
    Ok(
...
impl NodeLogRecovery {
    ...
    pub fn with_recovery_disk(mut self, recovery_disk: cellule_ltx::DiskBudget) -> Self {
...
struct TailReadContext<'a> {
    ...
    required_first: u64,
    limits: cellule_ltx::Limits,
    reservation: Option<&'a cellule_ltx::DiskReservation>,
...
async fn collect_member_tail(
    ...
    loop {
        ...
        let verified = match page
            .frames
            .into_iter()
            .map(|bytes| cellule_ltx::inspect_node_frame(bytes, limits))
            .collect::<cellule_ltx::Result<Vec<_>>>()
        {
...
fn default_recovery_disk(limits: cellule_ltx::Limits) -> cellule_ltx::DiskBudget {
    cellule_ltx::DiskBudget::new(recovery_tail_reservation_bytes(limits))
}

fn recovery_tail_reservation_bytes(limits: cellule_ltx::Limits) -> u64 {
...
mod tests {
    ...
    fn scope(cell: u8, sequence: u64) -> cellule_ltx::NodeFrameScope {
    ...
    async fn active_lane_requires_and_returns_a_complete_follower_tail() {
        ...
        let mut database =
            cellule_ltx::Db::open(&source.path().join("cell.sqlite"), limits).unwrap();
        database
        ...
        .unwrap()
        .with_recovery_disk(cellule_ltx::DiskBudget::new(0));
        assert!(rejected.ensure_sealed().await.is_err());
        let budget = cellule_ltx::DiskBudget::new(1 << 30);
        ...
        .unwrap()
        .with_recovery_disk(cellule_ltx::DiskBudget::new(1 << 30))
    ...
    async fn recovery_uses_the_next_complete_witness_after_a_read_failure() {
        ...
        for wrong_epoch in [false, true] {
            ...
            let replacement = cellule_ltx::encode_node_frame(
                scope,
                frame.segment().clone(),
                frame.body().clone(),
                limits,
