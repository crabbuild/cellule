...
impl cellule_ltx::bundle::BundleLease for ArtifactEntry {}
...
impl RecoveryArtifactStore for RecoveryArtifactRegistry {
    fn retain(&self, key: RecoveryArtifactKey, bundle: cellule_ltx::bundle::Bundle) -> Result<()> {
        ...
        if bundle.digest() != digest {
            return Err(Error::Ltx(cellule_ltx::LtxError::ChecksumMismatch));
        }
            ...
            .cloned()
        {
            let valid = existing.size == size
                && cellule_ltx::bundle::Bundle::decode_file(&existing.path, self.limits)
    ...
    fn load(&self, key: &RecoveryArtifactKey) -> Result<Option<RecoveryArtifact>> {
        ...
        let bundle = match cellule_ltx::bundle::Bundle::decode_file(&entry.path, self.limits) {
...
mod tests {
    ...
    use cellule_ltx::{Limits, Position, bundle::BundleBuilder, bundle::BundleEntry};
    use tempfile::TempDir;

    fn bundle_fixture(directory: &Path, marker: u8) -> cellule_ltx::bundle::Bundle {
        let database_path = directory.join(format!("database-{marker}"));
        let mut database = cellule_ltx::Db::open(&database_path, Limits::default()).unwrap();
        database
    ...
    fn key(bundle: &cellule_ltx::bundle::Bundle, marker: u8) -> RecoveryArtifactKey {
