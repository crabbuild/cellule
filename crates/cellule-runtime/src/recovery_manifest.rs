...
pub struct RecoveryArtifact {
    bundle: cellule_ltx::bundle::Bundle,
    lease: Arc<dyn cellule_ltx::bundle::BundleLease>,
}
...
impl RecoveryArtifact {
    ...
    fn into_parts(
        ...
        cellule_ltx::bundle::Bundle,
        Arc<dyn cellule_ltx::bundle::BundleLease>,
    ) {
...
impl RecoveryManifestStore {
    #[must_use]
    pub fn new(layout: CellStorageLayout, limits: cellule_ltx::Limits) -> Self {
        let recovery_disk = cellule_ltx::DiskBudget::new(limits.max_plan_bytes);
        Self {
            layout,
            limits,
            recovery_disk,
    ...
    pub async fn load_overlay(
        ...
        if let Some(artifacts) = &self.artifact_store {
            ...
            if let Ok(Some(artifact)) = cached {
                ...
                return Ok(cellule_ltx::RecoveryOverlay::new(
                    row.predecessor,
                    bundle,
                    row.final_position,
                    row.final_commit_sequence,
...
struct ManifestCell {
    ...
    first_node_sequence: u64,
    last_node_sequence: u64,
    predecessor: cellule_ltx::RootRef,
    final_position: cellule_ltx::Position,
    final_commit_sequence: u64,
    bundle_digest: [u8; 32],
}
...
fn runtime_root(root: cellule_ltx::RootRef) -> RootRef {
    RootRef {
...
mod tests {
    ...
    async fn recovery_fixture_with_store(
        ...
        let replica =
            cellule_ltx::CellReplica::new(layout.clone(), cell, incarnation, limits).unwrap();
