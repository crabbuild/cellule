...
pub struct RecoveredCellTail {
    ...
    pub overlay: cellule_ltx::RecoveryOverlay,
}
...
pub fn build_recovery_overlays(
    ...
    let mut grouped = BTreeMap::<Key, Vec<cellule_ltx::VerifiedNodeFrame>>::new();
    for frame in frames {
    ...
    for (key, mut cell_frames) in grouped {
        ...
        recovered.push(RecoveredCellTail {
            ...
            overlay: cellule_ltx::RecoveryOverlay::new(
                base.root,
                bundle,
    ...
    I: IntoIterator<Item = Result<cellule_ltx::VerifiedNodeFrame>>,
{
    ...
    for frame in frames {
        ...
        bundle.push(cellule_ltx::bundle::BundleEntry::for_cell(
            base.root.cell,
            base.root.incarnation,
            frame.segment().clone(),
            frame.body().to_vec(),
...
mod tests {
    ...
    fn recovery_witness_splits_interleaved_cells_against_exact_bases() {
        ...
        let mut left =
            cellule_ltx::Db::open(&directory.path().join("left.sqlite"), limits).unwrap();
        let mut right =
            cellule_ltx::Db::open(&directory.path().join("right.sqlite"), limits).unwrap();
    ...
    fn recovery_witness_splits_two_interleaved_cuts_from_one_thousand_cells() {
        ...
        let directory = tempfile::TempDir::new().unwrap();
        let mut database =
            cellule_ltx::Db::open(&directory.path().join("source.sqlite"), limits).unwrap();
        database
            .transaction(|transaction| {
                transaction.execute_batch(
                    "CREATE TABLE events(id INTEGER PRIMARY KEY, body TEXT NOT NULL);\
