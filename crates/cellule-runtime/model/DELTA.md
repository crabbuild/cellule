...
| `CoordinationInput::BeginTransferPreflight`/`ConfirmTransfer`/`AbortTransfer` and `CoordinationState::transfer_preparing` | `Admit` guards plus `Release`/`Takeover` | A prepared transfer collapses to an admission-closing quiesce state: confirmation is the modeled release and abort restores admission | A Cell cannot accept new work while its exact generation is being handed over, and an aborted preflight cannot release ownership |
| `inspect_transfer_work`/`TransferWorkInventory` | omitted | The transfer-specific indexed durable-work probe is a SQLite guard behind the modeled release action | Release still requires settled executable work; exact table and index coverage stays in Rust |
