...
The standalone SQL, KV, Queue, Workflow/Activity, Cron/Effect, and Timer deadline examples and the eight-Cell reference application exercise SQL, KV, Blob, Queue, Workflow, Activity, Cron, Timer, and Effect primitives. Keep examples compiled and verify their visible outcomes. Read `PERFORMANCE.md` before changing ignored benchmarks; preserve measured-run provenance. Run `cargo test -p cellule-app --locked` after changes.
