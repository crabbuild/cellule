...
The standalone SQL, KV, Queue, Queue consumer, Workflow/Activity, Cron/Effect, and Timer deadline examples and the nine-Cell reference application exercise SQL, KV, Blob, Queue, consumer, Workflow, Activity, Cron, Timer, Effect, and Projection primitives. Keep examples compiled and verify their visible outcomes. Read `PERFORMANCE.md` before changing ignored benchmarks; preserve measured-run provenance. Run `cargo test -p cellule-app --locked` after changes.
