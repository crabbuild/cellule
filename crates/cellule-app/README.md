...
Start with the [single-Cell orders example](examples/orders.rs): `cargo run -p cellule-app --example orders --locked`. The [reference application](tests/reference_application.rs) models orders, carts, attachments, notifications, fulfillment, and invoices across SQL, KV, Blob, Queue, Workflow, Activity, Cron, and Effect primitives. [Run the guide](../../docs/quickstart.md) for both local paths and their visible results. [Performance examples](PERFORMANCE.md) add three-runtime and three-process workloads with explicit limits on what they measure.
