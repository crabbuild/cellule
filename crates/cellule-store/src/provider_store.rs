...
pub const STATIC_ENV_PROVIDER_ENV: &str = "CELLULE_STORAGE_PROVIDER";
...
pub enum StaticEnvStoreUrlForm {
    ...
    Raw,
    /// Opaque application URL whose provider selection is supplied by the caller.
    Opaque,
}
...
/// Static-env store target plus the effective object prefix for routing.
#[derive(Debug, Clone, PartialEq, Eq)]
...
pub fn static_env_target_selection(
    ...
    override_provider: Option<StorageProviderKind>,
    ...
    if parts.provider == StorageProviderKind::Azure && parts.form == StaticEnvStoreUrlForm::Raw {
        ...
        return Ok(StaticEnvStoreTargetSelection {
            ...
            object_prefix: effective_object_prefix(object_prefix, default_object_prefix),
        });
    }

    let provider = match parts.form {
        StaticEnvStoreUrlForm::Raw => parts.provider,
        StaticEnvStoreUrlForm::Opaque => match override_provider {
    ...
    Ok(StaticEnvStoreTargetSelection {
        ...
        object_prefix: effective_object_prefix(parts.prefix, default_object_prefix),
    })
}
...
pub fn static_env_target_selection_for_provider(
    ...
    static_env_target_selection(parts, Some(expected_provider), default_object_prefix)
}
...
fn effective_object_prefix(prefix: &str, default_object_prefix: &str) -> String {
    let prefix = prefix.trim_matches('/');
    if prefix.is_empty() {
        default_object_prefix.to_owned()
    } else {
        prefix.to_owned()
    }
}

/// Resolves the static-env provider from `CELLULE_STORAGE_PROVIDER`.
///
...
/// Builds an object store from the provider SDK's default environment chain.
...
/// Builds an object store for a normalized static-env target.
...
mod tests {
    ...
    #[test]
    fn static_env_target_selection_uses_override_provider_for_opaque_url_parts() {
        let selection = static_env_target_selection(
            StaticEnvStoreUrlParts {
                provider: StorageProviderKind::S3,
                form: StaticEnvStoreUrlForm::Opaque,
                bucket: "bucket",
                prefix: "org/repo",
            },
        ...
        assert_eq!(
            ...
            StaticEnvStoreTargetSelection {
                ...
                object_prefix: "org/repo".into(),
            }
        );
    }

    #[test]
    fn static_env_target_selection_for_provider_allows_opaque_url_parts() {
    ...
    #[test]
    fn static_env_target_selection_strips_raw_azure_container_from_object_prefix() {
        let selection = static_env_target_selection(
            StaticEnvStoreUrlParts {
    ...
    #[test]
    fn static_env_target_selection_uses_default_object_prefix_for_container_root() {
        let selection = static_env_target_selection(
            StaticEnvStoreUrlParts {
                provider: StorageProviderKind::Azure,
                form: StaticEnvStoreUrlForm::Raw,
        ...
        assert_eq!(selection.object_prefix, "primary/repo");
        assert_eq!(
            selection.target,
