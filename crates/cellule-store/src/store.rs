...
impl Store {
    ...
    /// applying again nests the policies. Cloud stores built by `cellule-store`
    ...
    /// create-only writes fail closed; The store never retries them as an
    ...
    /// create-if-absent contract protects mutable metadata and immutable content
